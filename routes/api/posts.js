                ...
                Post.findById(req.params.id)
                    .then(post=>{
                        ...
                        if(post.likes.filter(like=>like.user.toString()===req.user.id).length > 0){
                            return res.json({alreadyliked : 'You have already liked this post'});
                        }
...
// @access PRIVATE
router.post('/comments/:id/:comment_id', passport.authenticate('jwt', {session:false}), (req,res)=>{
    Post.findById(req.params.id)
        .then(post=>{
            if(!post.comments.filter(comment=>comment.user.toString()===req.user.id).length>0){
                return res.status(400).json({nocommentfound:'You have not commented on this post'});
            }
